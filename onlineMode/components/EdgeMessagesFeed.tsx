      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h3 className="text-lg font-bold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            {t("au_edge_device_messages")}
          </h3>
          <p className="text-xs text-slate-400 font-medium mt-1">{t("au_synced_from_iot_desc")}</p>
        </div>
      </div>
      <div className="space-y-6">
        {loading ? (
          <p className="text-center py-10 text-slate-300 font-bold uppercase tracking-widest text-xs animate-pulse">{t("au_loading")}</p>
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start gap-2">
                <h4 className="text-sm font-bold text-slate-900 group-hover:text-auth-accent-red transition-colors">
                  {msg.peer_nick || t("au_unknown_node")}
                </h4>
                {msg.priority === 'CRITICAL' && (
                  <span className="text-[9px] font-black uppercase tracking-widest text-red-500 bg-red-50 px-2 py-0.5 rounded-full shrink-0">{t("critical")}</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                <span className="text-[10px] font-bold text-slate-300">· {t("au_node_word")} {msg.node_id}</span>
                {msg.origin_node_id && (
                  <span className="text-[10px] font-bold text-sky-500">
                    {t("au_via_lora")}{msg.rssi != null ? ` · ${msg.rssi} dBm` : ""}{msg.distance_m != null ? ` · ~${msg.distance_m}m` : ""}
                  </span>
                )}
              </div>
            </div>
          ...
          <p className="text-center py-10 text-slate-300 font-bold uppercase tracking-widest text-xs">{t("au_no_edge_messages")}</p>
        )}
      </div>
    </div>
  );
