...
export default function PeopleNeedsPage() {
  ...
  const stypeLabels: Record<string, string> = {
    medical: t("c_need_medical"),
    rescue: t("c_need_rescue_needed"),
    supplies: t("c_need_supplies"),
    fire: t("c_need_fire"),
    shelter: t("c_need_shelter_needed"),
    other: t("c_need_other")
  };
  ...
  const combinedItems = useMemo(() => {
    const resourceItems = resources.map(r => ({
      ...
      title: r.name,
      description: `${t("c_resource_type_label")} ${r.rtype}. ${t("c_monitoring_inventory")}`,
      status: r.status,
      timestamp: new Date(r.updated_at).toLocaleString(),
      location: r.regions?.name || r.region_id || t("c_unknown_location"),
      category: r.rtype,
    ...
    const helpItems = helpRequests.map(s => {
      ...
      return {
        ...
        description: details + (people ? ` — ${people} ${t("c_people_affected_suffix")}` : ''),
        status: urgency === 'critical' ? 'critical' : urgency === 'high' ? 'low' : 'adequate',
        timestamp: new Date(s.created_at).toLocaleString(),
        location: s.city || t("c_unknown_location"),
        category: t("c_help_request_category"),
        quantity: people || "1",
        unit: people ? t("c_unit_people") : t("c_unit_request"),
  ...
  }, [resources, helpRequests, searchQuery, filterStatus, filterRegion, filterSource, t]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-12 md:px-8 md:py-20 mt-16 md:mt-20">
        <PageHeader
          title={t("c_community_needs_title")}
          description={t("c_community_needs_desc")}
          count={totalCritical}
          countLabel={t("c_active_needs_label")}
        />
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center mb-8">
          <div className="relative flex-1 w-full flex items-center">
            ...
            <input
              type="text"
              placeholder={t("c_search_needs_placeholder")}
              value={searchQuery}
          ...
          <div className="flex w-full md:w-auto items-center gap-3">
            ...
            >
              <option value="all">{t("c_all_sources")}</option>
              <option value="resource">{t("c_resources_only")}</option>
              <option value="help_request">{t("c_help_requests_only")}</option>
            </select>
          </div>
          <div className="flex w-full md:w-auto items-center gap-3">
            <div className="bg-gray-50 border border-gray-200 p-3 rounded-xl hidden md:block">
          ...
          <div className="flex w-full md:w-auto items-center gap-3">
            ...
            <select 
              ...
              className="w-full bg-gray-50 border border-gray-200 px-4 py-3 rounded-xl text-sm font-bold text-zinc-900 focus:outline-none cursor-pointer"
            >
              <option value="all">{t("c_all_urgency")}</option>
              <option value="critical">{t("critical")}</option>
              <option value="low">{t("c_high_low_stock")}</option>
              <option value="adequate">{t("c_moderate_adequate")}</option>
              <option value="surplus">{t("c_surplus")}</option>
            </select>
          </div>
        </div>
